...
Локальный менеджер профилей для OpenConnect. Приложение запускает встроенную web-панель на `127.0.0.1`, хранит VPN-профили в зашифрованном vault и после ввода 4-значного PIN позволяет быстро переключаться между профилями.
...
После запуска откроется локальная страница вида:

```text
http://127.0.0.1:49111
```

Если порт занят, приложение выберет свободный локальный порт и напечатает URL в терминал.
